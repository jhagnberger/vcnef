...
class PatchEncoding(nn.Module):
    ...
    def __init__(self, patch_size=2, in_chans=6, embed_dim=96):
        super().__init__()
        self.conv = nn.Sequential(nn.Conv3d(in_chans, embed_dim // 2, kernel_size=patch_size // 2, stride=patch_size // 2, bias=True),
                                  nn.GELU(),
                                  nn.Conv3d(embed_dim // 2, embed_dim, kernel_size=2, stride=2, bias=True))
...
class PatchDecoding(nn.Module):
    ...
    def __init__(self, patch_size=2, out_chans=3, embed_dim=96):
        super().__init__()
        self.conv = nn.Sequential(nn.ConvTranspose3d(embed_dim, embed_dim // 2, kernel_size=patch_size // 2, stride=patch_size // 2, bias=True),
                                  nn.GELU(),
                                  nn.ConvTranspose3d(embed_dim // 2, out_chans, kernel_size=2, stride=2, bias=True))
...
class VCNeFModel(nn.Module):
    """
    VCNeF for 3D time-dependent PDEs. The input spatial domain is divided into non-overlapping
    ...
    ---------
        ...
        patch_size_small: Size of small patches (default: 4x4x4)
        patch_size_large: Size of large patches (default: 16x16x16)
        n_transformer_blocks: Number of Transformer blocks
        n_modulation_blocks: Number of modulation/ VCNeF blocks
    """

    def __init__(self, num_channels=5, condition_on_pde_param=True, pde_param_dim=2,
        ...
        input_dim = 3 + num_channels + pde_param_dim if condition_on_pde_param else 3 + num_channels
        ...
        self.coordinate_encoding_small = LearnableFourierPositionalEncoding(2, 3, d_model // 2, 32, d_model // 2, 10)
        self.coordinate_encoding_large = LearnableFourierPositionalEncoding(2, 3, d_model // 2, 32, d_model // 2, 10)
        ...
        self.de_embedding_small = PatchDecoding(out_chans=num_channels, embed_dim=d_model, patch_size=patch_size_small)
        self.de_embedding_large = PatchDecoding(out_chans=num_channels, embed_dim=d_model, patch_size=patch_size_large)
        self.final_conv = nn.Conv3d(num_channels * 2, num_channels, 1, 1)

    def forward(self, x, grid, pde_param, t):
        # x shape: b, s_x, s_y, s_z, channels
        ...
        if self.condition_on_pde_param:
            pde_param = pde_param[:, None, None, None, :].repeat(1, x.size(1), x.size(2), x.size(3), 1)
        ...
        d_small = x_small.shape[4]
        h_large = x_large.shape[2]
        w_large = x_large.shape[3]
        d_large = x_large.shape[4]

        x_small = rearrange(x_small, 'b c h w d -> b (h w d) c')
        x_large = rearrange(x_large, 'b c h w d -> b (h w d) c')
       ...
       # Encode temporal coordinates
        ...
        points_small = torch.cat((grid[:, ::self.patch_size_small, ::self.patch_size_small, ::self.patch_size_small, None, :],
                                  grid[:, (self.patch_size_small-1)::self.patch_size_small, (self.patch_size_small-1)::self.patch_size_small, (self.patch_size_small-1)::self.patch_size_small, None, :]), dim=-2)
        spatial_coordinates_small = self.coordinate_encoding_small(rearrange(points_small, 'b h w d g c -> b (h w d) g c'))
        ...
        points_large = torch.cat((grid[:, ::self.patch_size_large, ::self.patch_size_large, ::self.patch_size_large, None, :],
                                  grid[:, (self.patch_size_large-1)::self.patch_size_large, (self.patch_size_large-1)::self.patch_size_large, (self.patch_size_large-1)::self.patch_size_large, None, :]), dim=-2)
        spatial_coordinates_large = self.coordinate_encoding_large(rearrange(points_large, 'b h w d g c -> b (h w d) g c'))
        ...
        x_small = x[:, :, :h_small*w_small*d_small, :]
        x_small = rearrange(x_small, 'b t (h w d) c -> (b t) c h w d', h=h_small, w=w_small, d=d_small)
        x_small = self.de_embedding_small(x_small)

        # Decode large patches
        x_large = x[:, :, h_small*w_small*d_small:, :]
        x_large = rearrange(x_large , 'b t (h w d) c -> (b t) c h w d', h=h_large, w=w_large, d=d_large)
        x_large = self.de_embedding_large(x_large )
        ...
        # Reshape
        x = rearrange(x, '(b t) c h w d -> b h w d t c', t=t.shape[1])

        # x shape: b, s_x, s_y, s_z, n_t, channels
        return x
